    ...
    <td><table width="690" border="0">
        <tr> 
		  ...
		  isset($text) && isset($author) && isset($date)){
			  $result = mysql_query("update lessons
			  set title='$title', meta_d='$meta_d',meta_k='$meta_k',
			  description = '$description', date = '$date', text = '$text', author = '$author' where
			  id = '$id'");
			  
			  if($result = 'true') {echo "<p>Your lesson has been successfully updated!</p>";}
			  else {echo "<p> An error occured while updating lessons</p>";}
		  }
		  else {
